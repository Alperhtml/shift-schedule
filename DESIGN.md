...
- Merge list: refused files stay in the list with their reason in `--error`, and a removal button, rather than being reduced to a count. Reasons wrap; they used to be cut off on a phone with no way to read them.
- A file that needs a decision (same person twice, another period, over the roster cap) disables the primary action and shows one line under the summary. A file that simply brings nothing (no shifts, no name) is listed and skipped.
- A dialog that offers several jobs lists them as rows, one click each, with nothing preselected and no confirm button: a default is a job somebody did not choose. Rows that would do nothing are not shown at all.
- `InfoDot` is the `i` beside a control. Hover, tap and keyboard all open it, one sentence, and it never swallows the click of the row it sits on.
- Splash: the Koç emblem is inline SVG, 30 wedges on one spiral, revealed in drawing order from the smallest middle wedge outwards at 38 ms apart, held, faded, repeating on a 2400 ms cycle. It is plain HTML and CSS in `index.html` so it paints before any script, and being inline it costs no request and lets the page stop it. On failure the wedges freeze whole at 35 percent opacity behind the message. `prefers-reduced-motion` shows the finished emblem with no animation. Source and regeneration scripts live in `koc-logo/` outside the repo.
- Segmented controls are `w-fit`. Inside a column they would otherwise stretch and leave dead track to the right of the last option.
...
- Export menu: the JSON item is tinted `--accent-tint` with a 1 px inset `--accent-ring` and an 11 px second line, because it is the one export the merge needs. At most one item per menu is highlighted; a second would cancel the first. A highlighted item cannot use the ordinary hover, which goes from nothing to `--accent-tint`, because it already sits on that tint: it hovers to `--accent-tint-strong` and its ring goes to full `--accent`. Painted over the menu, that is a 29 step on red against the plain rows' 24, so the highlight never costs the item its sense of being clickable. The import menu highlights `Programları birleştir` for the same reason: it is what that menu exists for.
